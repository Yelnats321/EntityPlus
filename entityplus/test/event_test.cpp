...
TEST_CASE("receiving", "[event]") {
    ...
    auto sub2 = em.subscribe<float>([&](float) { wasCalled = true; });
...
TEST_CASE("component event", "[event]") {
    ...
    int compsAdded = 0;
    evtMan.subscribe<component_added<entity_t, A>>([&](const auto& event) {
        compsAdded++;
        REQUIRE(event.component.x == 2);
    });
    int compsRemoved = 0;
    evtMan.subscribe<component_removed<entity_t, A>>([&](const auto& event) {
        compsRemoved++;
        REQUIRE(event.component.x == 2);
    });
